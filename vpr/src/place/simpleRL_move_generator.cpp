 ...
 *                                     *
 *                                     */
SimpleRLMoveGenerator::SimpleRLMoveGenerator(std::unique_ptr<GradientAgent>& agent) {
...
/*                                  *
 *                                  *
 *  Gradient agent implementation   *
 *                                  *
 *                                  */
GradientAgent::GradientAgent(size_t num_actions) {
    average_reward_ = 0;
    num_available_actions_ = num_actions;
    q_ = std::vector<float>(num_actions, 0.);
    exp_q_ = std::vector<float>(num_actions, 0.);
    ...
    set_action_prob();
    /*
    std::string file_name = "agent_info_";
    file_name.append(std::to_string(num_actions));    
    //agent_info_file_ = vtr::fopen("agent_info.txt", "a");
    agent_info_file_ = vtr::fopen(file_name.c_str(), "w");
    */
}

GradientAgent::~GradientAgent() {
    if (agent_info_file_) vtr::fclose(agent_info_file_);
}

size_t GradientAgent::propose_action() {
    set_action_prob();
    size_t action = 0;
    float p = vtr::frand();
    auto itr = std::lower_bound(cumm_action_prob_.begin(), cumm_action_prob_.end(), p);
...
void GradientAgent::set_action_prob() {
    //calculate the scaled and clipped explonential function for the estimated q value for each action
    std::transform(q_.begin(), q_.end(), exp_q_.begin(), scaled_clipped_exp);
    ...
    if (sum_q == 0.0) { //action probabilities need to be initialized with equal values
        ...
        for (size_t i = 0; i < num_available_actions_; ++i) {
            action_prob_[i] = exp_q_[i] / sum_q;
            if(action_prob_[i] > 0.85)
                action_prob_[i] = 0.85;
            else if(action_prob_[i] < 0.04)
                action_prob_[i] = 0.04;
        }
    }
...
void GradientAgent::set_step(float gamma, int move_lim) {
    if (gamma < 0) {
        exp_alpha_ = -1; //Use sample average
    } else {
        //
...
void GradientAgent::process_outcome(double reward, e_reward_function reward_fun) {
    ++num_action_chosen_[last_action_];
    if (reward_fun == RUNTIME_AWARE || reward_fun == WL_BIASED_RUNTIME_AWARE)
        reward /= time_elapsed_[last_action_];
    ...
    //Calculate the updated average reward
    float delta_q = step * (reward - average_reward_);
    average_reward_ += delta_q;
 
    //Update the estimated value of all actions
    float delta_preference = step * (reward - average_reward_);
    for (size_t i = 0; i < num_available_actions_; ++i) {
        if(i == last_action_)
            q_[i] += delta_preference * (1-action_prob_[i]);
        else
            q_[i] -= delta_preference * action_prob_[i];
    }
    ...
    if (agent_info_file_) {
        ...
        for (size_t i = 0; i < num_available_actions_; ++i) {
            fprintf(agent_info_file_, "%g", action_prob_[i]);
            if (i != num_available_actions_ - 1) {
                fprintf(agent_info_file_, ",");
            }
        }
